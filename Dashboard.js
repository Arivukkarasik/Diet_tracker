...
export default function Dashboard() {
  ...
  //     );
  //     setFoods(result.data.foodDtoList);
  //     console.log(result.data.foodDtoList);
  //   } catch (error) {
 ...
 foodItemDetails.map((foodInfo,index) => (
  ...
  function validateId(){
    let error = {};
    if(foodItemDetails.foodId === ""){
      error.foodId = "Food Id is required";
    }
    else if(foodItemDetails.foodId.lenght === 0){
      error.foodId = "Invalid Food Id.";
    }
    else if(!/^\d+$/.test(foodItemDetails.foodId)){
      error.foodId = " Doesn't number type";
    }
    return error;
  }
}
